      ...
      const fetchArsipKeluar = async () => {
        ...
        try {
          const res = await fetch(
            `/api/admin/arsipkeluar?search=${encodeURIComponent(searchQuery)}`
          );
          if (!res.ok) {
            if (res.status === 401) {
              router.push('/login');
          ...
          setArsipKeluarData(
            data.map((item: ArsipKeluar) => ({ ...item, isEditing: false }))
          );
        } catch (err: any) {
    ...
    <div className='container mx-auto p-4 dark:text-gray-200'>
      ...
      {loading ? (
        ...
        <p className='text-gray-600 dark:text-gray-400'>
          Tidak ada data arsip surat keluar.
        </p>
      ) : (
                  ...
                  <td className='py-4 px-6 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300'>
                    {surat.isEditing ? (
                      <input
                        ...
                        value={surat.indeks}
                        onChange={e =>
                          handleInputChange(e, surat._id, 'indeks')
                        }
                        className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-200 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700'
                      />
                    ) : (
                  ...
                  <td className='py-4 px-6 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300'>
                    {surat.isEditing ? (
                      <input
                        ...
                        value={surat.tujuan}
                        onChange={e =>
                          handleInputChange(e, surat._id, 'tujuan')
                        }
                        className='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-200 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700'
                      />
                    ) : (
