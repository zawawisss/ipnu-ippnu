  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/kaderisasi-crud?jenjangKader=${jenjangKader}`
      );
    ...
    try {
      ...
      const url =
        isEditing && formData._id
          ? `/api/kaderisasi-crud?id=${formData._id}`
          : '/api/kaderisasi-crud';
      ...
      if (!res.ok)
        throw new Error(
          isEditing
            ? `Gagal memperbarui data ${jenjangKader}`
            : `Gagal menambah data ${jenjangKader}`
        );
      setAlertMessage(
        isEditing
          ? `Berhasil memperbarui ${jenjangKader}`
          : `Berhasil menambah ${jenjangKader}`
      );
      setAlertColor('success');
      setShowAlert(true);
      onOpenChange();
      ...
      setAlertMessage(
        err.message || `Terjadi kesalahan menyimpan ${jenjangKader}`
      );
      setAlertColor('danger');
      setShowAlert(true);
    }
        ...
        <ModalContent>
          {onClose => (
            <>
              <ModalHeader>
                {isEditing
                  ? `Edit Peserta ${jenjangKader}`
                  : `Tambah Peserta ${jenjangKader}`}
              </ModalHeader>
              <ModalBody>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  ...
                  <Input
                    ...
                    onChange={e =>
                      setFormData({ ...formData, komisariat: e.target.value })
                    }
                    isRequired
                  />
                  <Input
                  ...
                  <Input
                    ...
                    type='date'
                    value={
                      typeof formData.tanggalMulai === 'string'
                    ...
                    onChange={e =>
                      setFormData({
                        ...formData,
                        tanggalMulai: e.target.value as any,
                      })
                    }
                    isRequired
                  />
                  <Input
                    ...
                    type='date'
                    value={
                      typeof formData.tanggalSelesai === 'string'
                    ...
                    onChange={e =>
                      setFormData({
                        ...formData,
                        tanggalSelesai: e.target.value as any,
                      })
                    }
                  />
                  <Input
                  ...
                  <Select
                    ...
                    onSelectionChange={keys => {
                      ...
                      setFormData({
                        ...formData,
                        organization: selectedOrg as 'IPNU' | 'IPPNU',
                      });
                    }}
                    isRequired
                  >
                  ...
                  <Select
                    ...
                    selectedKeys={
                      formData.statusKader ? [formData.statusKader] : []
                    }
                    onSelectionChange={keys => {
                      const selectedStatus = Array.from(keys).join('');
                      setFormData({
                        ...formData,
                        statusKader: selectedStatus as
                          | 'Aktif'
                          | 'Tidak Aktif'
                          | 'Alumni',
                      });
