...
export default function SidebarIPPNU() {
  ...
  const displayRole = session?.user?.role ? session.user.role.charAt(0).toUpperCase() + session.user.role.slice(1) : 'Guest';
  const displayOrg = session?.user?.name?.includes("ippnu") ? "IPPNU" : "IPNU";
  const displayUser = session?.user?.name ? `${displayOrg} ${displayRole}` : 'Loading...';
  ...
  const navItems = [
    ...
    { name: "Anggota", href: "/admin/anggota", icon: UserGroupIcon },
    {
      ...
      children: [
        { name: "Surat Masuk", href: "/admin_ippnu/surat/masuk", icon: ArrowLeftOnRectangleIcon },
        { name: "Surat Keluar", href: "/admin_ippnu/surat/keluar", icon: ArrowLeftOnRectangleIcon },
        { name: "Buat Surat", href: "/admin_ippnu/surat/buat", icon: ChartBarSquareIcon },
        { name: "Surat Menunggu Persetujuan", href: "/admin_ippnu/surat/tugas/approval", icon: ChartBarSquareIcon },
        { name: "Surat Disetujui", href: "/admin_ippnu/surat/tugas/approved", icon: ChartBarSquareIcon },
      ],
    },
  ];
    ...
    <aside className="sticky top-0 left-0 h-screen w-full sm:w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 z-50 flex flex-col transition-width duration-300">
      ...
      <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700">
        <div className="hidden sm:block mb-2 text-sm text-gray-500 dark:text-gray-400">
            <p>Login sebagai:</p>
            <p className="font-medium text-gray-700 dark:text-gray-300 truncate">{displayUser}</p>
        </div>
        <Button
            variant="ghost"
            size="sm"
