...
import {
  ...
  FileText,
  FileSpreadsheet,
} from 'lucide-react'; // Add FileSpreadsheet icon
...
export default function KeuanganForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const parsedJumlah = parseFloat(
      form.jumlah.replace(/\./g, '').replace(/,/g, '.')
    );
    ...
    try {
      ...
      fetchLaporan();
      setAlertMessage(
        `Data berhasil ${isEditing ? 'diperbarui' : 'disimpan'}!`
      );
  ...
  const getFilteredLaporan = () => {
    ...
    if (filterTanggalMulai) {
      filtered = filtered.filter(row =>
        dayjs(row.tanggal).isSameOrAfter(filterTanggalMulai, 'day')
      );
    }
    if (filterTanggalAkhir) {
      filtered = filtered.filter(row =>
        dayjs(row.tanggal).isSameOrBefore(filterTanggalAkhir, 'day')
      );
    }
  ...
  const totalDebet = filteredLaporan.reduce(
    (sum, row) => sum + (row.debet ?? 0),
    0
  );
  const totalKredit = filteredLaporan.reduce(
    (sum, row) => sum + (row.kredit ?? 0),
    0
  );
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `laporan_keuangan_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`
    );
      ...
      <Card className='p-4 md:p-6 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 text-gray-900 dark:text-gray-100'>
          ...
          <div className='p-4 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-sm'>
            ...
            <p
              className={`text-xl font-bold ${saldoAkhir >= 0 ? 'text-blue-600' : 'text-red-600'}`}
            >
