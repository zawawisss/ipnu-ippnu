...
import { Button, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, useDisclosure, Alert } from '@heroui/react';
...
const ImportKaderisasiButton: React.FC<ImportKaderisasiButtonProps> = ({ organization, onImportComplete }) => {
  ...
  const handleImport = async () => {
    if (!file) {
      setImportStatus({ type: 'error', message: 'Silakan pilih file Excel terlebih dahulu' });
      return;
    }
          ...
          {(onClose) => (
            <>
              ...
              <ModalBody>
                {importStatus && (
                  <Alert 
                    color={importStatus.type === 'success' ? 'success' : importStatus.type === 'error' ? 'danger' : 'primary'}
                    className="mb-4"
                  >
                ...
                <div className="space-y-4">
                  <div>
                    <h4 className="text-lg font-semibold mb-2">Panduan Import:</h4>
                    <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                      <li>• File harus berformat Excel (.xlsx)</li>
                      <li>• Sheet yang akan diproses: "DATA KADERISASI {organization}"</li>
                      <li>• Kolom yang diperlukan: TANGGAL, PENGKADERAN, PIMPINAN, TEMPAT, JUMLAH</li>
                      <li>• Data akan dikonversi menjadi data kader individual</li>
                    </ul>
                  </div>
