...
const EventList: React.FC<EventListProps> = ({ events, onEdit, onDelete }) => {
  ...
  return (
    <div className='bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl'>
      <h2 className='text-3xl font-extrabold mb-6 text-gray-900 dark:text-white border-b pb-3 border-gray-200 dark:border-gray-700'>
        ...
        <div className='space-y-8'>
          {events.map(event => (
            <div
              ...
              className='border border-gray-200 dark:border-gray-700 rounded-xl p-5 bg-gray-50 dark:bg-gray-700/50 shadow-md'
            >
              <div className='flex justify-between items-start'>
                <div>
                  ...
                  <p className='text-sm text-gray-500 dark:text-gray-400'>
                    {event.delegation ? `Delegasi: ${event.delegation}` : ''}
                  </p>
                </div>
              ...
              <div className='mt-4 pt-4 border-t border-gray-200 dark:border-gray-600'>
                ...
                <div className='grid grid-cols-2 gap-6'>
                  <div>
                    ...
                    <ul className='list-disc list-inside pl-2 text-xs text-gray-600 dark:text-gray-400'>
                      {(event.ipnuAttendees || []).length > 0 ? (
                        event.ipnuAttendees.map(name => (
                          <li key={name}>{name}</li>
                        ))
                      ) : (
                        <li>-</li>
