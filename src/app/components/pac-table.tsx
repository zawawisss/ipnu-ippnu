...
function PACTable() {
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/kecamatanList?page=${currentPage}&limit=${rowsPerPage}&searchTerm=${searchTerm}`
    )
      .then(res => res.json())
      .then(data => {
      ...
      <div className='flex flex-col sm:flex-row justify-between gap-4'>
        ...
        <div className='flex items-center gap-4'>
          <div className='flex items-center text-sm text-gray-500 dark:text-gray-400'>
            {kecamatanData.total} data ditemukan{' '}
          ...
          <TableBody>
            {isLoading ? (
              ...
              paginatedData.map(
                (
                  kec: any,
                  i: number // Use paginatedData for mapping
                ) => (
                  <TableRow
                    key={kec._id}
                  ...
                  >
                    <TableCell className='text-center'>
                      {(kecamatanData.page - 1) * kecamatanData.limit + i + 1}{' '}
                      {/* Use page and limit from API response */}
                    </TableCell>
                    ...
                    <TableCell className='text-center'>
                      {kec.tanggal_sp
                        ? new Date(kec.tanggal_sp).toLocaleDateString('id-ID', {
                            day: '2-digit',
                            month: 'long',
                            year: 'numeric',
                          })
