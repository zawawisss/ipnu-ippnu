  ...
  try {
    ...
    if (org === 'ipnu') {
      if (event.ipnuAttendees.includes(name)) {
        return NextResponse.json({ message: "Anda sudah terdaftar pada acara ini." }, { status: 400 });
      }
      if (event.ipnuAttendees.length >= 2) {
        return NextResponse.json({ message: "Kuota IPNU sudah penuh." }, { status: 400 });
      }
      ...
      if (event.ippnuAttendees.includes(name)) {
       ...
       if (event.ippnuAttendees.length >= 2) {
        return NextResponse.json({ message: "Kuota IPPNU sudah penuh." }, { status: 400 });
      }
