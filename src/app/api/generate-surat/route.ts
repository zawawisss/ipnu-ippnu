...
function generateNomorSurat({ urut, tingkatan = 'PC', kodeSurat = 'ST', periodisasi = 'XXXI', tanggal = new Date() }: { urut: number, tingkatan?: string, kodeSurat?: string, periodisasi?: string, tanggal?: Date }) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!data || Object.keys(data).length === 0) {
      console.error('Received data is empty or null.');
    ...
    try {
      ...
      return NextResponse.json({ error: `Rendering error: ${renderError.message}` }, { status: 500 });
    }
