...
function extractPimpinanInfo(
  pimpinan: string | null
): { type: string; base: string } | null {
...
export async function GET() {
  try {
    ...
    const combinedAndGroupedData = Array.from(groupedData.values()).map(
      entry => {
        ...
        if (
          ipnuInfo &&
          ippnuInfo &&
          ipnuInfo.base === ippnuInfo.base &&
          ipnuInfo.type === ippnuInfo.type
        ) {
